drop table SSH_BUILD_DETAIL;
drop table SSH_BUILD_RECOMMEND;
drop table SSH_REPLY;
drop table SSH_POTENTIAL;

drop table SSH_BUILD;
drop table SSH_CHARACTER;
drop table SSH_MEMBER;
...
select *
  from ssh_build b
       join ssh_build_detail bd on b.no = bd.build_no
       join ssh_potential p on bd.char_no = p.char_no and bd.pot_no = p.no
       join ssh_character c on c.no = bd.char_no
;
